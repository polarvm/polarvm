...
#ifndef POLAR_BASIC_ADT_EQUIVALENCE_CLASSES_H
#define POLAR_BASIC_ADT_EQUIVALENCE_CLASSES_H
...
///    for (EquivalenceClasses<int>::member_iterator MI = EC.memberBegin(I);
...
public:
   ...
   const EquivalenceClasses &operator=(const EquivalenceClasses &rhs)
   {
      ...
      for (iterator iter = rhs.begin(), end = rhs.end(); iter != end; ++iter) {
         if (iter->isLeader()) {
            member_iterator miter = rhs.memberBegin(iter);
            member_iterator leaderIt = memberBegin(insert(*miter));
   ...
   member_iterator memberBegin(iterator I) const {
      // Only leaders provide anything to iterate over.
      return member_iterator(I->isLeader() ? &*I : nullptr);
   }
   member_iterator memberEnd() const
   {
   ...
   {
      member_iterator miter = findLeader(value);
   ...
   {
      member_iterator miter = findLeader(insert(value));
      assert(miter != memberEnd() && "Value is not in the set!");
      return *miter;
   }
   ...
   member_iterator findLeader(iterator iter) const
   {
      if (iter == m_theMapping.end()) {
         return memberEnd();
      }
      return member_iterator(iter->getLeader());
   }

   member_iterator findLeader(const ElemType &value) const
   {
   ...
   member_iterator unionSets(const ElemType &lhs, const ElemType &rhs)
   {
      iterator lhsIter = insert(lhs), rhsIter = insert(rhs);
      return unionSets(findLeader(lhsIter), findLeader(rhsIter));
   }

   member_iterator unionSets(member_iterator lhs, member_iterator rhs)
   {
   ...
   public:
      ...
      explicit member_iterator(const ECValue *node) : m_node(node)
      {}
...
#endif // POLAR_BASIC_ADT_EQUIVALENCE_CLASSES_H
