...
//
// KeyType objects are mapped to ValueType objects. Intervals of keys that map to the
...
//===----------------------------------------------------------------------===//
//
#ifndef POLAR_BASIC_ADT_INTERVAL_MAP_H
#define POLAR_BASIC_ADT_INTERVAL_MAP_H
...
#include "polar/utils/RecyclingAllocator.h"
#include <algorithm>
#include <cassert>
#include <cstdint>
#include <iterator>
...
private:
   ...
   template <typename NodeType> NodeType *newNode()
   {
      return new(m_allocator.template allocate<NodeType>()) NodeType();
   }

   template <typename NodeType> void deleteNode(NodeType *node)
   {
      node->~NodeType();
...
#endif // POLAR_BASIC_ADT_INTERVAL_MAP_H
