...
#ifndef POLAR_BASIC_ADT_INTRUSIVE_LIST_ITERATOR_H
...
template <class OptionsType, bool IsConst>
...
template <class OptionsType> struct IteratorTraits<OptionsType, true> {
...
struct IteratorHelper<false> : ilist_internal::NodeAccess
{
   ...
   template <class T> static void increment(T *&iter)
   {
      iter = Access::getNext(*iter);
   }
   
   template <class T> static void decrement(T *&iter)
   {
...
template <class OptionsType, bool IsReverse, bool IsConst>
...
#endif // POLAR_BASIC_ADT_INTRUSIVE_LIST_ITERATOR_H
