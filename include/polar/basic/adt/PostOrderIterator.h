...
#ifndef POLAR_BASIC_ADT_POST_ORDER_ITERATOR_H
#define POLAR_BASIC_ADT_POST_ORDER_ITERATOR_H
...
public:
   ...
   template <class NodeRef> bool insertEdge(std::optional<NodeRef> from, NodeRef to)
   {
      return m_visited.insert(to).second;
   }

   // Called after all children of node have been visited.
   template <class NodeRef> void finishPostorder(NodeRef node)
   {}
};
...
template <class T> IteratorRange<PostOrderIterator<T>> post_order(const T &graph)
{
...
#endif // POLAR_BASIC_ADT_POST_ORDER_ITERATOR_H
