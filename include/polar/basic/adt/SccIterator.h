...
#ifndef POLAR_BASIC_ADT_SCCITERATOR_H
#define POLAR_BASIC_ADT_SCCITERATOR_H
...
template <class GraphT, class GT> void SccIterator<GraphT, GT>::getNextScc()
{
...
template <class T> SccIterator<T> scc_begin(const T &graph)
{
   return SccIterator<T>::begin(graph);
}

/// \brief Construct the end iterator for a deduced graph type T.
template <class T> SccIterator<T> scc_end(const T &graph)
{
...
#endif // POLAR_BASIC_ADT_SCCITERATOR_H
