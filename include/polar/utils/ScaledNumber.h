...
#ifndef POLAR_UTILS_SCALED_NUMBER_H
#define POLAR_UTILS_SCALED_NUMBER_H
...
get_rounded(DigitsT m_digits, int16_t m_scale, bool shouldround)
{
   ...
   if (shouldround) {
      if (!++m_digits) {
         // Overflow.
         return std::make_pair(DigitsT(1) << (get_m_width<DigitsT>() - 1), m_scale + 1);
      }
   }
                                                ...
                                                int16_t m_scale = 0)
{
   ...
   const int m_width = get_m_width<DigitsT>();
...
inline std::pair<DigitsT, int16_t> getProduct(DigitsT lhs, DigitsT rhs)
{
   ...
   if (get_m_width<DigitsT>() <= 32 || (lhs <= UINT32_MAX && rhs <= UINT32_MAX)) {
...
///
/// Returns \c INT32_MIN when \c m_digits is zero.
template <class DigitsT> int32_t get_lg(DigitsT m_digits, int16_t m_scale)
{
...
///
/// Returns \c INT32_MIN when \c m_digits is zero.
template <class DigitsT> int32_t get_lg_ceiling(DigitsT m_digits, int16_t m_scale)
{
   auto lg = get_lg_impl(m_digits, m_scale);
   return lg.first + (lg.second < 0);
}
                     ...
                     int16_t &rhsScale)
{
   ...
   if (scaleDiff >= 2 * get_m_width<DigitsT>()) {
   ...
   assert(shiftL < get_m_width<DigitsT>() && "can't shift more than m_width");
   int32_t ShiftR = scaleDiff - shiftL;
   if (ShiftR >= get_m_width<DigitsT>()) {
      // Don't bother shifting.  rhsDigits will get zero-ed out anyway.
      rhsDigits = 0;
      return lhsScale;
   }
                                    ...
                                    DigitsT rhsDigits, int16_t rhsScale)
{
   ...
   DigitsT highBit = DigitsT(1) << (get_m_width<DigitsT>() - 1);
   return std::make_pair(highBit | sum >> 1, m_scale + 1);
}

/// \brief Convenience helper for 32-bit sum.
inline std::pair<uint32_t, int16_t> get_sum32(uint32_t lhsDigits, int16_t lhsScale,
...
inline std::pair<uint64_t, int16_t> get_sum64(uint64_t lhsDigits, int16_t lhsScale,
...
#endif // POLAR_UTILS_SCALED_NUMBER_H
