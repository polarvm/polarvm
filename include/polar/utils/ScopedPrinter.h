...
#ifndef POLAR_UTILS_SCOPED_PRINTER_H
#define POLAR_UTILS_SCOPED_PRINTER_H

#include "polar/global/DataTypes.h"
#include "polar/basic/adt/ApSInt.h"
...
struct EnumEntry
{
   ...
   EnumEntry(StringRef name, StringRef altName, T value)
      : m_name(name), m_altName(altName), m_value(value)
   {}

   EnumEntry(StringRef name, T value)
      : m_name(name), m_altName(name), m_value(value)
   {}
};
...
struct DelimitedScope
{
   ...
   DelimitedScope(ScopedPrinter &printer, StringRef str) : m_printer(printer)
   {
      ...
      if (!str.empty()) {
         m_printer.getOStream() << ' ';
      }
      m_printer.getOStream() << Open << '\n';
      m_printer.indent();
   }
...
#endif // POLAR_UTILS_SCOPED_PRINTER_H
