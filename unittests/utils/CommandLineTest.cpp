...
template <typename T>
class StackOption : public cmd::Opt<T>
{
   typedef cmd::Opt<T> Base;
public:
   // One option...
   template<class M0t>
...
TEST(CommandLineTest, AliasesWithArguments) {
   ...
   for (size_t i = 0, e = array_lengthof(Inputs); i < e; ++i) {
      ...
      cmd::parse_command_line_options(ARGC, Inputs[i]);
      EXPECT_EQ("x", Actual);
...
void testAliasRequired(int argc, const char *const *argv)
{
   ...
   cmd::parse_command_line_options(argc, argv);
   EXPECT_EQ("x", Option);
...
TEST(CommandLineTest, testSetValueInSubcategories)
{
   ...
   EXPECT_TRUE(
            cmd::parse_command_line_options(2, args, StringRef(), &null_stream()));
   ...
   EXPECT_TRUE(
            cmd::parse_command_line_options(3, args2, StringRef(), &null_stream()));
   EXPECT_FALSE(TopLevelOpt);
   EXPECT_TRUE(SC1Opt);
   EXPECT_FALSE(SC2Opt);
   ...
   EXPECT_TRUE(
            cmd::parse_command_line_options(3, args3, StringRef(), &null_stream()));
   EXPECT_FALSE(TopLevelOpt);
   EXPECT_FALSE(SC1Opt);
   EXPECT_TRUE(SC2Opt);
}
...
TEST(CommandLineTest, testLookupFailsInWrongSubCommand)
{
   ...
   EXPECT_FALSE(cmd::parse_command_line_options(3, args, StringRef(), &outstream));
   outstream.flush();
...
TEST(CommandLineTest, testAddToAllSubCommands)
{
   ...
   EXPECT_FALSE(AllOpt);
   EXPECT_TRUE(cmd::parse_command_line_options(2, args, StringRef(), &outstream));
   EXPECT_TRUE(AllOpt);
   ...
   cmd::reset_all_option_occurrences();
   EXPECT_FALSE(AllOpt);
   EXPECT_TRUE(cmd::parse_command_line_options(3, args2, StringRef(), &outstream));
   EXPECT_TRUE(AllOpt);
   ...
   cmd::reset_all_option_occurrences();
   EXPECT_FALSE(AllOpt);
   EXPECT_TRUE(cmd::parse_command_line_options(3, args3, StringRef(), &outstream));
   EXPECT_TRUE(AllOpt);
...
TEST(CommandLineTest, RemoveFromRegularSubCommand) {
   ...
   EXPECT_TRUE(cmd::parse_command_line_options(3, args, StringRef(), &outstream));
...
TEST(CommandLineTest, RemoveFromAllSubCommands) {
   ...
   EXPECT_FALSE(RemoveOption);
   EXPECT_TRUE(
            cmd::parse_command_line_options(2, args0, StringRef(), &null_stream()));
   EXPECT_TRUE(RemoveOption);
   ...
   EXPECT_FALSE(RemoveOption);
   EXPECT_TRUE(
            cmd::parse_command_line_options(3, args1, StringRef(), &null_stream()));
   EXPECT_TRUE(RemoveOption);
...
TEST(CommandLineTest, GetRegisteredSubcommands) {
   ...
   cmd::reset_all_option_occurrences();
   EXPECT_TRUE(
            cmd::parse_command_line_options(2, args1, StringRef(), &null_stream()));
   EXPECT_FALSE(Opt1);
   EXPECT_FALSE(Opt2);
   for (auto *S : cmd::get_registered_subcommands()) {
      if (*S) {
...
TEST(CommandLineTest, ArgumentLimit)
{
   ...
   EXPECT_FALSE(sys::command_line_fits_within_system_limits("cmd", args.data()));
}
